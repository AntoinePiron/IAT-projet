...
class QAgent(AgentInterface):
    ...
    def __init__(self, SpaceInvader: SpaceInvaders, eps_profile: EpsilonProfile, gamma: float, alpha: float):
        ...
        :attribut SpaceInvadersValues: la fonction de valeur stockée qui sera écrite dans un fichier de log après la résolution complète
        :type SpaceInvadersValues: data frame pandas
        ...
        self.Q = np.zeros([SpaceInvaders.ny, SpaceInvaders.nx, SpaceInvaders.na])
        ...
        self.values = pd.DataFrame(data={'nx': [SpaceInvaders.nx], 'ny': [SpaceInvaders.ny]})
    ...
    def learn(self, env, n_episodes, max_steps):
        ...
        for episode in range(n_episodes):
            ...
            state = env.reset_using_existing_SpaceInvaders()
            # Execute K steps 
    ...
    def save_log(self, env, episode):
        ...
        V = np.zeros((int(self.SpaceInvaders.ny), int(self.SpaceInvaders.nx)))
        for state in self.SpaceInvaders.getStates():
        ...
        self.values = self.values.append({'episode': episode, 'value': np.reshape(V,(1, self.SpaceInvaders.ny*self.SpaceInvaders.nx))[0]},ignore_index=True)
